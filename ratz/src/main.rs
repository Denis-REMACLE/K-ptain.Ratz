...
async fn heartbeat (username_string: &String, srv_pub_key: RsaPublicKey, mut rng: OsRng) -> OwnedWriteHalf {
    loop{
        ...
        if message_must_sended {
            ...
            let _enc_data = srv_pub_key.encrypt(&mut rng, PaddingScheme::new_pkcs1v15_encrypt(), &json_message.as_bytes()).expect("failed to encrypt");
        }
    }
}
